...
use mavlink::common::{MavMessage, MavCmd, COMMAND_LONG_DATA};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let forward_addr: SocketAddr = args.forward_addr.parse()
    ...
    let socket_10124 = Arc::new(Mutex::new(UdpSocket::bind(format!("0.0.0.0:{}", listen_port)).await?));
    ...
    let task1 = tokio::spawn(async move {
        ...
        loop {
            ...
            match socket.recv_from(&mut buf).await {
                Ok((size, source_addr)) => {
                    debug!("Received {} bytes from {} on port {}", size, source_addr, listen_port_task1);
                    ...
                    match forward_socket_clone.send_to(&buf[..size], forward_addr_task1).await {
                        Ok(_) => {
                            debug!("Forwarded {} bytes from {} to {}", size, source_addr, forward_addr_task1);
                        }
                        Err(e) => {
    ...
    let task2 = tokio::spawn(async move {
        ...
        loop {
            match forward_socket.recv_from(&mut buf).await {
                Ok((size, _)) => {
                    debug!("Received {} bytes response from {}", size, forward_addr_task2);
                ...
                Err(e) => {
                    error!("Error receiving response from {}: {}", forward_addr_task2, e);
                }
            }
        }
    });
...
async fn trigger_camera_command(
    ...
    match write_v2_msg(&mut buffer, header, &msg) {
        Ok(_) => {
            ...
            match socket.send_to(&buffer, addr).await {
                Ok(_) => {
                    info!("Sent COMMAND_LONG message to {} (sequence={}) via HTTP trigger", addr, sequence);
                    Ok(Json(json!({
                        "status": "success",
...
async fn set_camera_mode(
    ...
    match write_v2_msg(&mut buffer, header, &msg) {
        Ok(_) => {
            ...
            match socket.send_to(&buffer, addr).await {
                Ok(_) => {
                    info!("Sent set_camera_mode command to {} (mode={}, sequence={})", addr, payload.mode, sequence);
                    Ok(Json(json!({
                        "status": "success",
...
async fn video_click(
    ...
    match payload.command_type.as_str() {
        "Tracking" => {
            ...
            info!("Tracking command sent: x={}, y={}, channel={}", x_pos, y_pos, payload.channel_id);
        }
        "RefineLocation" => {
            ...
            info!("RefineLocation command sent: x={}, y={}, channel={}", x_pos, y_pos, payload.channel_id);
        }
        "Both" => {
            // Send tracking first
            ...
            info!("Both commands sent: x={}, y={}, channel={}", x_pos, y_pos, payload.channel_id);
        }
        _ => {
            return Err(StatusCode::BAD_REQUEST);
        }
